  ...
  Widget _buildSidePanel(EssayState state) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.border.withOpacity(0.8)),
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 18, offset: const Offset(0, 6))],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Progress Essay', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: AppColors.text)),
          const SizedBox(height: 8),
          ClipRRect(
            borderRadius: BorderRadius.circular(8),
            child: LinearProgressIndicator(value: progress / 100, minHeight: 8, backgroundColor: AppColors.border.withOpacity(0.7), valueColor: const AlwaysStoppedAnimation<Color>(Color(0xFF4F8CFF))),
          ),
          const SizedBox(height: 8),
          Text('${progress.toStringAsFixed(0)}% • Soal ${state.currentQuestionIndex + 1} dari ${state.totalQuestions}', style: const TextStyle(fontSize: 12, color: AppColors.textLight)),
          const SizedBox(height: 4),
          Text('Tersimpan: ${state.savedCount}/${state.totalQuestions}', style: const TextStyle(fontSize: 12, color: AppColors.textLight)),
          const SizedBox(height: 12),
          Container(
            width: double.infinity, padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(color: const Color(0xFFF3F6FF), borderRadius: BorderRadius.circular(10)),
            child: Text(state.isDraftSaved ? 'Draft jawaban sudah disimpan' : 'Draft belum disimpan', style: const TextStyle(fontSize: 12, color: AppColors.textLight)),
          ),
          const SizedBox(height: 10),
          Text('Karakter: ${state.currentAnswer.length}', style: const TextStyle(fontSize: 12, color: AppColors.textLight)),
          const SizedBox(height: 4),
          Text('Kata: ${state.currentWordCount}', style: const TextStyle(fontSize: 12, color: AppColors.textLight)),
        ],
      ),
    );
  }
        ...
        children: [
          Container(
            ...
            child: Row(
              children: [
                Container(
                  width: 34, height: 34, decoration: const BoxDecoration(color: AppColors.primary, shape: BoxShape.circle),
                ...
                Expanded(child: Text('Pertanyaan ${state.currentQuestionIndex + 1} dari ${state.totalQuestions}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.text))),
              ],
            ),
          ),
          const SizedBox(height: 14),
          Container(
            ...
            child: Text(state.currentQuestion, style: const TextStyle(fontSize: 15, height: 1.6, color: AppColors.text)),
          ),
          const SizedBox(height: 16),
          const Text('Jawaban Anda', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.text)),
          const SizedBox(height: 8),
          Container(
          ...
          Row(
            children: [
              Text('${state.currentWordCount} kata (min 10) | ${state.currentAnswer.length} karakter', style: const TextStyle(fontSize: 11, color: AppColors.textLight)),
              const Spacer(),
              Text(state.isDraftSaved ? 'Draft tersimpan' : 'Belum disimpan', style: TextStyle(fontSize: 11, color: state.isDraftSaved ? AppColors.success : AppColors.textLighter)),
            ],
          ),
          const SizedBox(height: 12),
  ...
  Widget _buildBottomActionBar(EssayState state) {
    ...
    return SafeArea(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white, border: Border(top: BorderSide(color: AppColors.border.withOpacity(0.9))),
          boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.12), blurRadius: 22, offset: const Offset(0, -8), spreadRadius: 1)],
        ),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(width: 42, height: 4, margin: const EdgeInsets.only(bottom: 10), decoration: BoxDecoration(color: AppColors.border, borderRadius: BorderRadius.circular(999))),
            Row(
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(side: BorderSide(color: AppColors.border.withOpacity(0.9)), padding: const EdgeInsets.symmetric(vertical: 14), backgroundColor: Colors.white, foregroundColor: AppColors.text),
                  ),
                ),
                ...
                Expanded(
                  child: ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, shadowColor: AppColors.primary.withOpacity(0.45), elevation: 8, padding: const EdgeInsets.symmetric(vertical: 14)),
