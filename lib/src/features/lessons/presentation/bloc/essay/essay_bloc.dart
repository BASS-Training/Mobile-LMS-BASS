...
class EssayBloc extends Bloc<EssayEvent, EssayState> {
  ...
  EssayBloc({
    ...
    on<ChangeQuestion>((event, emit) {
      ...
      final currentAnswer = state.workingAnswers[state.currentQuestionIndex] ?? '';
      repository.saveDraftAnswer(state.lessonId, state.currentQuestionIndex, currentAnswer);
    ...
    on<SubmitEssayClicked>((event, emit) async {
      ...
      if (isSuccess) {
        ...
        emit(state.copyWith(
          isSubmitting: false,
          errorMessage: 'Masih ada nomor yang belum memenuhi minimal ${SubmitEssayUseCase.minWordsPerQuestion} kata.',
        ));
      }
    });
